...
class TOOL:
    def __init__(self, number, cwIo, ccwIo=None):
        ...
        if not (isinstance(cwIo, DIGITAL_OUTPUT_STATE)):
            raise Exception(
                "IO for clockwise direction must be an instance of DIGITAL_OUTPUT_STATE. Received: {}".format(cwIo)
            )
        self.number = number
        self.cwIo = cwIo
        if ccwIo:
            if not (isinstance(ccwIo, DIGITAL_OUTPUT_STATE)):
                raise Exception(
                    "IO for counter-clockwise direction must be an instance of DIGITAL_OUTPUT_STATE. Received: {}".format(ccwIo)
                )
            self.ccwIo = ccwIo
...
class MachineMotion(object):
    ...
    def startPath(self, path, maxAcceleration, speedOverride=1.0):
        """
    ...
    def getPathStatus(self):
        ...
        exampleCodePath: pathFollowing.py
        """
        lResponse = self.myGCode.__sendToSmartDrives__("/smartDrives/path/status",JsonResponse=True)
